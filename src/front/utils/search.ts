...
export function simplesearch(search: JQuery) {
  const input = $(search.find('input').get(0));
  const container = $(`#${search.attr('s-con')}`);

  const actualize = () => {
    let val = input.val() as string;
    if (typeof val === 'undefined') return;
    val = val.toLowerCase();
    const li = container.find('*[s-val]');
    li.each(i => {
      const e = $(li.get(i));
      if (val === '') {
        e.css('display', '');
      } else {
        let elemval = e.attr('s-val');
        if (elemval) {
          ...
          if (elemval.startsWith(val)) {
            ...
            if (
              elemval.includes('.') &&
              elemval.split('.')[1].startsWith(val)
            ) {
              e.css('display', '');
            } else {
              e.css('display', 'none');
