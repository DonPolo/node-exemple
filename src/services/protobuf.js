...
function jsonToStructProto(json) {
  ...
  Object.keys(json).forEach(k => {
    fields[k] = jsonValueToProto(json[k]); // eslint-disable-line no-use-before-define
  });
...
function structProtoToJson(proto) {
  ...
  const json = {};
  Object.keys(proto.fields).forEach(k => {
    json[k] = valueProtoToJson(proto.fields[k]); // eslint-disable-line no-use-before-define
  });
  return json;
}
