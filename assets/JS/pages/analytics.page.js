...
class BottomBar extends ParentComponent {
  ...
  render () {
    ...
    let swi = <input onChange={controller.toggleArchive} type="checkbox" name="archived" id="toggle-archive" checked={this.props.archived ? true: false}/>
    let prev = null;
    if (this.props.curpage > 1) {
      prev = <a onClick={() => controller.changePageNoOption(this.props.curpage-1)}><i className="fas fa-angle-left"></i></a>
    }
    let next = null;
    if (this.props.curpage < this.props.nbpage) {
      next = <a onClick={() => controller.changePageNoOption(this.props.curpage+1)}><i className="fas fa-angle-right"></i></a>
    }
    let pages = [];
    if (this.props.nbpage > 0) {
      for(let i = 1; i <= this.props.nbpage; i++) {
        if (i == this.props.curpage) {
          pages.push(<a key={i} onClick={() => controller.changePageNoOption(i)} className='enable'>{ i }</a>)
        } else {
          pages.push(<a key={i} onClick={() => controller.changePageNoOption(i)}>{ i }</a>)
        }
      }
    }
    return (
      <div className="bottom-bar">
        <span>
          <form onSubmit={() => controller.inputPageSubmit(event, this)}>
            Page:
            <input type="text" name="page" value={ this.state.selectedpage } onChange={() => controller.inputPage(event, this)}/>
          </form>
        </span>
...
class Data extends ParentComponent {
  ...
  render () {
    ...
    if (!this.props.data.archived) {
      action = <span><i className="fas fa-trash" archive={ this.props.data._id } onClick={() => controller.archiveData(this.props.data._id)}></i></span>
    } else {
      action = <span><i className="fas fa-trash-restore" recover={ this.props.data._id } onClick={() => controller.recoverData(this.props.data._id)}></i></span>
    }
    ...
    return (
      <>
      <div className="accordion" onClick={accordion.noAnim} id="{ this.props.data._id }">
        <span>Query: <i>"{ this.props.data.request.result.query }"</i></span>
...
class Analytics extends ParentComponent {
  ...
  render () {
    ...
    const datas = this.props.datas.map((data, index) => <Data key={index} data={data}/>)
    return (
      <main>
        {datas}
      </main>
  ...
  render () {
    return (
      <>
        ...
        <BottomBar nbpage={this.state.nbpage} archived={this.state.archived} curpage={this.state.curpage}/>
      </>
    );
  }
}
